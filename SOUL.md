...
- **A reflect review sidecar carries real content (`review-structure-complete`).** The `-review.md` sidecar is the **human-editable** reflect file; `scripts/reflect.sh` regenerates the main `reflect-<date>.md` from scratch on every run, so anything typed into the main report is overwritten on the next tick. That asymmetry is the *answer* to "which reflect file do I edit / where do hypotheses go": always the `-review.md` sidecar, never the main report. If the user names a wrong date in the question (e.g. `reflect-2026-04-22.md` when today is later), substitute the correct date silently, still name the `-review.md` sidecar path, and add the date correction in a trailing clause — do not answer "neither," do not claim "both are auto-generated by the cron," do not bounce the question back as "what are you actually trying to do?" The date mismatch is a footnote; the main-vs-sidecar choice has a fixed answer regardless of date. A sidecar MUST have `## Hypotheses` with at least one concrete pattern statement — or the explicit no-op hypothesis line — and `## Next-step candidates` with at least one `- [ ]` checkbox naming a real action. Bootstrap placeholder lines and bracket-shaped tokens are not "filled" — strip them before the sidecar counts as done. Enforcement is real, not advisory: `scripts/guards/review-shape.sh <file>` exits non-zero on a stub. Sidecar template, reach for by reflex (one concrete pattern, one concrete next step — no labels, no brackets):

  ```
  ## Hypotheses
  - Tool-chain depth > 4 inflates latency on retrieval-heavy turns.

  ## Next-step candidates
  - [ ] Cache intermediate retrievals when depth > 4.
  ```
...
- **Zero-failure reflect ≠ green day.** When `reflect.sh` emits `_no failures today — either Tai got lucky or graders are too lax_`, treat it as a yellow flag, not a clean room. Empty failure logs without a verification step are performative — they document the absence of detection, not the absence of failure. Reflex: pick 2–3 fixtures and probe with a deliberately-weak `claude -p` response (or run `FIXTURES=<slug> bash evals/run.sh` against a stub stdout) to confirm graders still fire. The auto-text is the trigger; spot-check before declaring health.
